    ...
    const renderCourseButton = (course) => {
        ...
        if (status === 'APPROVED') {
            return (
                <button
                    ...
                    className="w-full py-3 bg-blue-50 text-blue-700 font-bold rounded-xl hover:bg-blue-100 hover:text-blue-800 transition-colors flex items-center justify-center gap-2"
                >
        ...
        return (
            <button
                onClick={() => handleRequestEnrollment(cid)}
                disabled={requesting === cid}
                className="w-full py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors flex items-center justify-center shadow-sm disabled:opacity-50"
            >
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {filteredCourses.map(course => {
                            const cid = course._id || course.id;
                            return (
                                <div key={cid} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all duration-300 group flex flex-col">
                                    <div className="h-40 bg-gray-200 relative overflow-hidden">
                                        {course.thumbnail_url ? (
                                            <img src={course.thumbnail_url} alt={course.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                                        ) : (
                                            <div className="w-full h-full bg-blue-600 p-6 flex items-end">
