            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        <div className="mt-4">
                            <div className="flex items-center gap-3">
                                <div className="flex-1 flex gap-1">
                                    {[...Array(5)].map((_, i) => (
                                        <div key={i} className={`h-6 flex-1 rounded ${i < Math.min(exams.length, 5) ? 'bg-emerald-400' : 'bg-gray-100'}`}></div>
                                    ))}
                                </div>
                ...
                <section className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    {loading ? (
                        ...
                        (() => {
                            const now = new Date();
                            const assignedExams = exams.filter(e => !e.is_blocked && new Date(e.start_time) <= now);
                            const upcomingExams = exams.filter(e => !e.is_blocked && new Date(e.start_time) > now);
                            const completedExams = exams.filter(e => e.is_blocked);
