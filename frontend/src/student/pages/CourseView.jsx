...
const QuizSection = ({ quiz, courseId, existingScore, onComplete }) => {
    ...
    return (
        <div className="bg-white rounded-xl border border-indigo-100 p-6 mt-6">
                        ...
                        <div key={idx} className="mb-5 p-4 bg-gray-50 rounded-lg">
                            ...
                            <div className="space-y-2">
                                {q.options.map((opt, optIdx) => (
                                    <label key={optIdx} className={`flex items-center gap-3 p-2.5 rounded-lg cursor-pointer border transition-colors ${answers[idx] === optIdx ? 'bg-indigo-50 border-indigo-300' : 'bg-white border-gray-200 hover:bg-gray-50'}`}>
                                        <input type="radio" name={`q-${idx}`} checked={answers[idx] === optIdx} onChange={() => setAnswers(prev => ({ ...prev, [idx]: optIdx }))} className="accent-indigo-600" />
...
const CodingSection = ({ problem, courseId }) => {
    ...
    return (
        <div className="bg-white rounded-xl border border-emerald-100 p-6 mt-6">
            ...
            <textarea
                ...
                className="w-full h-48 p-3 font-mono text-sm bg-gray-900 text-green-400 rounded-lg border border-gray-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none"
                spellCheck="false"
            />

            <button onClick={handleRun} disabled={running}
                className="mt-3 px-6 py-2.5 bg-emerald-600 text-white rounded-lg font-semibold hover:bg-emerald-700 transition disabled:opacity-50 flex items-center gap-2">
                {running ? (
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-10 max-w-md text-center">
                    ...
                    <button onClick={() => navigate('/courses')} className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition font-medium text-sm">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="hidden sm:flex items-center gap-2 bg-indigo-50 px-3 py-1.5 rounded-full">
                        <div className="w-24 h-2 bg-gray-200 rounded-full overflow-hidden">
                            <div className="h-full bg-indigo-600 rounded-full transition-all" style={{ width: `${progress?.progress_percentage || 0}%` }}></div>
                        </div>
                        <span className="text-xs font-semibold text-indigo-700">{Math.round(progress?.progress_percentage || 0)}%</span>
                    </div>
                        ...
                        {course.modules.map((mod, mIdx) => (
                            <div key={mod.id} className="mb-3">
                                <button
                                    ...
                                    className={`w-full text-left px-3 py-2.5 rounded-lg text-sm font-semibold transition ${mIdx === activeModuleIdx ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-gray-50'}`}
                                >
                                    {mod.title}
                                    ...
                                    <div className="ml-3 mt-1 space-y-0.5">
                                        {mod.lessons.map((lesson, lIdx) => (
                                            <button
                                                ...
                                                className={`w-full text-left px-3 py-2 rounded-md text-xs flex items-center gap-2 transition ${lIdx === activeLessonIdx ? 'bg-indigo-100 text-indigo-800 font-medium' : 'text-gray-600 hover:bg-gray-50'}`}
                                            >
                                        ...
                                        {mod.quizzes?.length > 0 && (
                                            <div className="px-3 py-1.5 text-xs text-indigo-500 font-medium flex items-center gap-1.5">
                                        ...
                                        {mod.coding_problems?.length > 0 && (
                                            <div className="px-3 py-1.5 text-xs text-emerald-500 font-medium flex items-center gap-1.5">
                ...
                <main className="flex-1 overflow-y-auto">
                    ...
                    <div className="max-w-5xl mx-auto px-6 py-6">
                        <div className="flex items-start justify-between">
                            <div>
                                <p className="text-xs text-indigo-600 font-semibold uppercase tracking-wide">{currentModule?.title}</p>
