                                ...
                                <div className="mt-1 max-h-32 overflow-y-auto border border-gray-200 rounded-lg bg-white shadow-sm">
                                    {filteredStudents.slice(0, 5).map(s => (
                                        <button
                                            ...
                                            className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 ${selectedStudent === (s._id || s.id) ? 'bg-indigo-50' : ''}`}
                                        >
            ...
            <div className="flex gap-2 mb-4">
                {['PENDING', 'APPROVED', 'REJECTED'].map(s => (
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition ${statusFilter === s ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                    >
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {enrollments.map(e => (
                                <tr key={e.id} className="hover:bg-gray-50 transition">
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-500">{new Date(e.requested_at).toLocaleDateString()}</td>
                                    <td className="px-6 py-4">
