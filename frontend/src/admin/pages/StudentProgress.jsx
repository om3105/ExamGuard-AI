                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h2 className="text-2xl font-bold text-gray-900">{studentDetail.student.full_name}</h2>
                                    <p className="text-gray-500 text-sm">{studentDetail.student.email} · {studentDetail.student.college || 'N/A'}</p>
                                </div>
                            </div>
                            {studentDetail.high_risk_count > 0 && (
                                <div className="mt-4 bg-red-50 border border-red-200 rounded-lg px-4 py-2 flex items-center gap-2">
                                    <AlertTriangle className="w-4 h-4 text-red-500" />
                                    <span className="text-sm font-medium text-red-700">{studentDetail.high_risk_count} high-risk exam submission(s)</span>
                                </div>
                            )}
                        </div>
                                    ...
                                    {studentDetail.course_progress.map((cp, i) => (
                                        <div key={i} className="border border-gray-100 rounded-lg p-4">
                                            <div className="flex items-center justify-between mb-2">
                                                ...
                                                <span className="text-sm font-bold text-blue-600">{cp.progress_percentage}%</span>
                                            </div>
                                            <div className="w-full bg-gray-100 rounded-full h-2.5 mb-2">
                                                <div className="bg-blue-500 h-2.5 rounded-full transition-all" style={{ width: `${cp.progress_percentage}%` }}></div>
                                            </div>
            ...
            {overview && (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 mb-8">
                    {[
                        ...
                        { label: 'Avg Completion', value: `${overview.avg_course_completion}%`, icon: TrendingUp, color: 'text-green-600 bg-green-50' },
                        { label: 'Avg Quiz Score', value: `${overview.avg_quiz_score}%`, icon: Award, color: 'text-amber-600 bg-amber-50' },
                        { label: 'Avg Coding Score', value: `${overview.avg_coding_score}%`, icon: Code, color: 'text-purple-600 bg-purple-50' },
                        { label: 'Exam Submissions', value: overview.total_exam_submissions, icon: BarChart3, color: 'text-orange-600 bg-orange-50' },
                    ].map((m, i) => {
                        const Icon = m.icon;
                        return (
