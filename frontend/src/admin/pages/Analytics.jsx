    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
                {[
                    { label: 'Total Exams', value: overview?.total_exams || 0, icon: FileText, bg: 'bg-blue-50', text: 'text-blue-600', border: 'border-blue-100' },
                    { label: 'Total Students', value: overview?.total_students || 0, icon: Users, bg: 'bg-emerald-50', text: 'text-emerald-600', border: 'border-emerald-100' },
                    { label: 'Submissions', value: overview?.total_submissions || 0, icon: TrendingUp, bg: 'bg-violet-50', text: 'text-violet-600', border: 'border-violet-100' },
                    { label: 'Avg Risk Score', value: overview?.avg_anomaly_score || 0, icon: Activity, bg: 'bg-amber-50', text: overview?.avg_anomaly_score > 50 ? 'text-red-600' : 'text-amber-600', border: 'border-amber-100' },
                    { label: 'High Risk', value: overview?.high_risk_submissions || 0, icon: AlertTriangle, bg: 'bg-red-50', text: 'text-red-600', border: 'border-red-100' },
                ].map(({ label, value, icon: Icon, bg, text, border }) => (
                    <div key={label} className={`${bg} rounded-xl p-5 border ${border}`}>
